...
export function PolicyForm({ teams, defaultTeamId, escalationPolicies = [], initialData }: PolicyFormProps) {
  ...
  async function buildCsvPreview(file: File) {
    ...
    try {
      ...
      const overlapCandidates = new Map<string, Array<{ line: number; startsAt: Date; endsAt: Date }>>();
      const dayCollisionMap = new Map<string, { line: number; startsAt: Date; endsAt: Date }>();
      const rowErrorMap = new Map<number, string[]>();

      const buildLocalDayKeys = (startsAt: Date, endsAt: Date): string[] => {
        if (!(endsAt > startsAt)) {
          const key = new Intl.DateTimeFormat("en-CA").format(startsAt);
          return [key];
        }
        ...
        while (cursor.getTime() < endsAt.getTime() && guard < 400) {
          const day = new Intl.DateTimeFormat("en-CA").format(cursor);
          if (!seen.has(day)) {
            seen.add(day);
            keys.push(day);
          }
        ...
        const tailKey = new Intl.DateTimeFormat("en-CA").format(endProbe < startsAt ? startsAt : endProbe);
        if (!seen.has(tailKey)) {
          keys.push(tailKey);
        }
      ...
      for (const row of parsed.rows) {
        ...
        if (assigneeResolved && startsAt && endsAt) {
          ...
          const dayKeys = buildLocalDayKeys(startsAt, endsAt);
          for (const dayKey of dayKeys) {
            const collisionKey = `${assigneeResolved.id}|${dayKey}`;
            const existing = dayCollisionMap.get(collisionKey);
            if (existing) {
              rowErrors.push(
                `assignee "${assigneeRaw}" trùng ngày ${dayKey} với dòng ${existing.line} (${formatPreviewTime(existing.startsAt)} - ${formatPreviewTime(existing.endsAt)})`
              );
              break;
            }
            dayCollisionMap.set(collisionKey, { line: row.line, startsAt, endsAt });
          }
        }

        rowErrorMap.set(row.line, rowErrors);
        previewRows.push({
          line: row.line,
      ...
      for (const entries of overlapCandidates.values()) {
        ...
        for (let idx = 1; idx < sorted.length; idx += 1) {
          ...
          if (current.startsAt < previous.endsAt && current.endsAt > previous.startsAt) {
            ...
            const message = `trùng thời gian với dòng ${previous.line}: ${formatPreviewTime(overlapStart)} - ${formatPreviewTime(overlapEnd)}`;
            const rowErrors = rowErrorMap.get(current.line) ?? [];
            rowErrors.push(message);
            rowErrorMap.set(current.line, rowErrors);
          }
        }
      }

      for (const row of previewRows) {
        row.errors = rowErrorMap.get(row.line) ?? [];
