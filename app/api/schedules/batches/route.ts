import { NextRequest } from "next/server";
import { z } from "zod";
import { prisma } from "@/lib/prisma";
import { getSessionUser, requireTeamRole, isNextResponse } from "@/lib/rbac";
import {
  created,
  unauthorized,
  badRequest,
  conflict,
  notFound,
  handleError,
} from "@/lib/api-response";
import { ShiftStatus, ShiftSource, TeamRole } from "@/app/generated/prisma/client";
import {
  computeConfirmationDueAt,
  TimeSlot,
  PriorState,
  localDayKey,
  isNightShiftTime,
} from "@/lib/rotation/engine";
import { writeAuditLog } from "@/lib/audit";
import { scheduleAllRemindersForBatchSafe } from "@/lib/queue/scheduler";
import { notifyTeamChannels } from "@/lib/notifications/notify-channel";
import { notifyAssigneesScheduleUpdated } from "@/lib/notifications/notify-assignees";
import { rateLimit, RATE_LIMITS } from "@/lib/rate-limit";
import { addWeeks, subDays } from "date-fns";
import {
  filterTeamMembersByPolicySelection,
  getPolicyParticipantUserIds,
} from "@/lib/rotation/policy-participants";
import {
  buildOccupiedMapFromShifts,
  generateAutoShiftsWithoutConflict,
} from "@/lib/rotation/auto-schedule-rebalance";
import type { AutoScheduleWarning } from "@/lib/rotation/auto-schedule-rebalance";
import { buildAutoScheduleWarningNote } from "@/lib/rotation/auto-schedule-warning";

const PublishSchema = z.object({
  policyId: z.string().uuid(),
  rangeStart: z.string().datetime(),
  rangeEnd: z.string().datetime().optional(),
  weeks: z.number().int().min(1).max(52).optional(),
  mode: z.enum(["AUTO", "MANUAL"]).optional(),
  manualAssignments: z.array(
    z.object({
      assigneeId: z.string().uuid(),
      backupId: z.string().uuid().nullable().optional(),
      startsAt: z.string().datetime(),
      endsAt: z.string().datetime(),
      notes: z.string().max(500).optional(),
    })
  ).optional(),
});

export async function POST(req: NextRequest) {
  const limited = await rateLimit(req, RATE_LIMITS.WRITE);
  if (limited) return limited;

  const idempotencyKey = req.headers.get("Idempotency-Key");
  if (!idempotencyKey) {
    return badRequest("Idempotency-Key header is required");
  }

  try {
    const actor = await getSessionUser();
    if (!actor) return unauthorized();

    const body = await req.json();
    const data = PublishSchema.parse(body);

    // Idempotency: return existing batch if key already used
    const existing = await prisma.scheduleBatch.findUnique({
      where: { idempotencyKey },
    });
    if (existing) {
      return created(existing);
    }

    const policy = await prisma.rotationPolicy.findUnique({
      where: { id: data.policyId },
      include: {
        team: {
          include: {
            members: {
              include: {
                user: { select: { id: true } },
              },
              orderBy: { order: "asc" },
            },
          },
        },
      },
    });

    if (!policy) return notFound("Policy not found");
    if (!policy.isActive) {
      return conflict("Policy is inactive. Reactivate or create a new policy.", "POLICY_INACTIVE");
    }

    const result = await requireTeamRole(policy.teamId, TeamRole.MANAGER);
    if (isNextResponse(result)) return result;

    if (policy.team.members.length === 0) {
      return badRequest("Team has no members to assign shifts to");
    }

    const selectedParticipantUserIds = await getPolicyParticipantUserIds(policy.id);
    const eligibleMembers = filterTeamMembersByPolicySelection(policy.team.members, selectedParticipantUserIds);
    if (eligibleMembers.length === 0) {
      return badRequest("Chính sách này chưa có thành viên áp dụng");
    }

    const participants = eligibleMembers.map((m, i, arr) => ({
      userId: m.user.id,
      backupId: arr[(i + 1) % arr.length].user.id,
    }));
    const participantUserIdSet = new Set(participants.map((p) => p.userId));

    const mode = data.mode ?? ((data.manualAssignments?.length ?? 0) > 0 ? "MANUAL" : "AUTO");

    type ShiftDraft = {
      assigneeId: string;
      backupId?: string | null;
      startsAt: Date;
      endsAt: Date;
      notes?: string;
    };

    let rangeStart: Date;
    let rangeEnd: Date;
    let shiftsToCreate: ShiftDraft[] = [];
    let shiftSource: ShiftSource = ShiftSource.AUTOGENERATED;
    let autoWarning: AutoScheduleWarning | null = null;

    if (mode === "MANUAL") {
      const assignments = data.manualAssignments ?? [];
      if (assignments.length === 0) {
        return badRequest("manualAssignments is required for manual mode");
      }

      shiftsToCreate = assignments.map((assignment) => ({
        assigneeId: assignment.assigneeId,
        backupId: assignment.backupId ?? undefined,
        startsAt: new Date(assignment.startsAt),
        endsAt: new Date(assignment.endsAt),
        notes: assignment.notes,
      }));

      for (const shift of shiftsToCreate) {
        if (shift.endsAt <= shift.startsAt) {
          return badRequest("Manual shift endsAt must be after startsAt");
        }
        if (!participantUserIdSet.has(shift.assigneeId)) {
          return badRequest("Manual shift assignee must be selected in this policy");
        }
      }

      rangeStart = shiftsToCreate.reduce(
        (min, shift) => (shift.startsAt < min ? shift.startsAt : min),
        shiftsToCreate[0].startsAt
      );
      rangeEnd = shiftsToCreate.reduce(
        (max, shift) => (shift.endsAt > max ? shift.endsAt : max),
        shiftsToCreate[0].endsAt
      );
      shiftSource = ShiftSource.MANUAL;
    } else {
      rangeStart = new Date(data.rangeStart);
      rangeEnd = data.rangeEnd
        ? new Date(data.rangeEnd)
        : addWeeks(rangeStart, data.weeks ?? policy.maxGenerateWeeks);

      if (rangeEnd <= rangeStart) {
        return badRequest("rangeEnd must be after rangeStart");
      }
    }

    // Check for overlapping published batch on the same policy
    const overlap = await prisma.scheduleBatch.findFirst({
      where: {
        policyId: data.policyId,
        status: "PUBLISHED",
        rangeStart: { lt: rangeEnd },
        rangeEnd: { gt: rangeStart },
      },
    });
    if (overlap) {
      return conflict(
        "A published batch overlaps the requested date range. Roll it back first.",
        "BATCH_OVERLAP"
      );
    }

    if (mode === "AUTO") {
      // Continue rotation from where last batch left off
      const previousShiftCount = await prisma.shift.count({
        where: {
          policyId: data.policyId,
          source: ShiftSource.AUTOGENERATED,
          status: { notIn: [ShiftStatus.CANCELLED] },
        },
      });
      const timeSlots = policy.timeSlots as TimeSlot[] | null | undefined;
      const slotsPerDay = timeSlots && timeSlots.length > 0 ? timeSlots.length : 1;
      // For time-slot mode, rotation advances 1 position per day (not per shift),
      // so startingIndex = number of previously scheduled days % participants.
      // For non-time-slot mode, it advances per shift as before.
      const startingIndex = participants.length > 0
        ? timeSlots && timeSlots.length > 0
          ? Math.floor(previousShiftCount / slotsPerDay) % participants.length
          : previousShiftCount % participants.length
        : 0;

      const existingTeamShifts = await prisma.shift.findMany({
        where: {
          // Check conflicts across all policies/teams for selected assignees,
          // not only within the current team.
          assigneeId: { in: participants.map((p) => p.userId) },
          status: { in: [ShiftStatus.PUBLISHED, ShiftStatus.ACTIVE, ShiftStatus.COMPLETED] },
          startsAt: { lt: rangeEnd },
          endsAt: { gt: rangeStart },
        },
        select: { id: true, assigneeId: true, policyId: true, startsAt: true, endsAt: true },
      });
      const occupied = buildOccupiedMapFromShifts(existingTeamShifts);

      // Carry over prior state so consecutive-night and rest-day rules hold across batch boundaries.
      const priorState = await buildPriorState(data.policyId, rangeStart, timeSlots, policy.timezone);

      try {
        const autoScheduleResult = generateAutoShiftsWithoutConflict({
          policy: { ...policy, timeSlots },
          policyId: data.policyId,
          participants,
          rangeStart,
          rangeEnd,
          startingIndex,
          timezone: policy.timezone,
          occupied,
          existingShifts: existingTeamShifts,
          priorState,
        });

        autoWarning = autoScheduleResult.warning;
        shiftsToCreate = autoScheduleResult.shifts.map((s) => ({
          assigneeId: s.assigneeId,
          backupId: s.backupId,
          startsAt: s.startsAt,
          endsAt: s.endsAt,
          notes: s.hasWarning ? buildAutoScheduleWarningNote() : undefined,
        }));
      } catch {
        return badRequest(
          "Không thể sắp xếp đủ ca trực mà vẫn đảm bảo mỗi người chỉ có 1 ca trong ngày."
        );
      }
    }

    // Create batch + shifts + confirmations in a single transaction
    const batch = await prisma.$transaction(async (tx) => {
      const newBatch = await tx.scheduleBatch.create({
        data: {
          policyId: data.policyId,
          publishedBy: actor.id,
          rangeStart,
          rangeEnd,
          idempotencyKey,
        },
      });

      await tx.shift.createMany({
        data: shiftsToCreate.map((s) => ({
          policyId: data.policyId,
          batchId: newBatch.id,
          assigneeId: s.assigneeId,
          backupId: s.backupId,
          startsAt: s.startsAt,
          endsAt: s.endsAt,
          status: ShiftStatus.PUBLISHED,
          source: shiftSource,
          notes: s.notes ?? null,
        })),
      });

      // Create confirmations for each shift
      const createdShifts = await tx.shift.findMany({
        where: { batchId: newBatch.id },
        select: { id: true, assigneeId: true, startsAt: true, endsAt: true },
      });

      const batchCreatedAt = new Date();
      const minDueAt = new Date(batchCreatedAt.getTime() + 60 * 60 * 1000); // at least 1h from now
      await tx.shiftConfirmation.createMany({
        data: createdShifts.map((s) => {
          const computed = computeConfirmationDueAt(
            { assigneeId: s.assigneeId, startsAt: s.startsAt, endsAt: s.endsAt },
            policy.confirmationDueHours
          );
          return {
            shiftId: s.id,
            userId: s.assigneeId,
            dueAt: computed < minDueAt ? minDueAt : computed,
          };
        }),
      });

      const tasks = (policy as any).templateTasks as string[];
      if (tasks && tasks.length > 0) {
        await tx.shiftTask.createMany({
          data: createdShifts.flatMap((s) =>
            tasks.map((title, order) => ({ shiftId: s.id, title, order }))
          ),
        });
      }

      return newBatch;
    });

    // Fetch confirmations to schedule reminders
    const confirmations = await prisma.shiftConfirmation.findMany({
      where: { shift: { batchId: batch.id } },
      include: { shift: { select: { startsAt: true, endsAt: true } } },
    });

    const remindersScheduled = await scheduleAllRemindersForBatchSafe(
      confirmations.map((c) => ({
        id: c.id,
        shiftId: c.shiftId,
        userId: c.userId,
        dueAt: c.dueAt,
        shift: c.shift,
      })),
      policy,
      `publish-batch:${batch.id}`
    );

    await writeAuditLog({
      actorId: actor.id,
      entityType: "ScheduleBatch",
      entityId: batch.id,
      action: "PUBLISH",
      newValue: {
        policyId: data.policyId,
        rangeStart,
        rangeEnd,
        shiftCount: shiftsToCreate.length,
        mode,
        autoWarning,
      },
      ipAddress: req.headers.get("x-forwarded-for") ?? undefined,
    });

    // Notify team Telegram channels that a new schedule has been published
    notifyTeamChannels({
      teamId: policy.teamId,
      eventType: "SCHEDULE_PUBLISHED",
      templateId: "schedule-published",
      recipientId: actor.id,
      variables: {
        actorName: actor.fullName,
        policyName: policy.name,
        shiftCount: shiftsToCreate.length.toString(),
        rangeStart: rangeStart.toISOString(),
        rangeEnd: rangeEnd.toISOString(),
        appUrl: process.env.NEXT_PUBLIC_APP_URL ?? "",
      },
    }).catch((e) => console.error("notify team channels failed:", e));

    // Personal Telegram notice for assignees, independent from queue workers.
    const assigneeNotifications = await notifyAssigneesScheduleUpdated({
      policyName: policy.name,
      shifts: confirmations.map((c) => ({
        assigneeId: c.userId,
        startsAt: c.shift.startsAt,
        endsAt: c.shift.endsAt,
        confirmationId: c.id,
      })),
      reason: "published",
    });

    return created({
      ...batch,
      remindersScheduled,
      assigneeNotifications,
      autoWarning,
    });
  } catch (error) {
    return handleError(error);
  }
}

async function buildPriorState(
  policyId: string,
  rangeStart: Date,
  timeSlots: TimeSlot[] | null | undefined,
  timezone: string | null | undefined
): Promise<PriorState> {
  if (!timeSlots || timeSlots.length === 0) return {};

  const tz = timezone ?? "Asia/Ho_Chi_Minh";
  const lookback = subDays(rangeStart, 3);

  const recentShifts = await prisma.shift.findMany({
    where: {
      policyId,
      source: ShiftSource.AUTOGENERATED,
      status: { in: [ShiftStatus.PUBLISHED, ShiftStatus.ACTIVE, ShiftStatus.COMPLETED] },
      startsAt: { gte: lookback, lt: rangeStart },
    },
    select: { assigneeId: true, startsAt: true, endsAt: true },
    orderBy: { startsAt: "asc" },
  });

  if (recentShifts.length === 0) return {};

  const last = recentShifts[recentShifts.length - 1];
  const previousAssigneeId = last.assigneeId;

  const lastNightShift = [...recentShifts]
    .reverse()
    .find((s) => isNightShiftTime(s.startsAt, s.endsAt, tz));
  const previousNightAssigneeId = lastNightShift?.assigneeId ?? null;

  const dayBeforeKey = localDayKey(subDays(rangeStart, 1), tz);
  const nightYesterdayIds = [...new Set(
    recentShifts
      .filter(
        (s) =>
          localDayKey(s.startsAt, tz) === dayBeforeKey &&
          isNightShiftTime(s.startsAt, s.endsAt, tz)
      )
      .map((s) => s.assigneeId)
  )];

  const nightYesterday = recentShifts.find(
    (s) =>
      localDayKey(s.startsAt, tz) === dayBeforeKey &&
      isNightShiftTime(s.startsAt, s.endsAt, tz)
  );

  const twoDaysBeforeKey = localDayKey(subDays(rangeStart, 2), tz);
  const nightTwoDaysAgoIds = [...new Set(
    recentShifts
      .filter(
        (s) =>
          localDayKey(s.startsAt, tz) === twoDaysBeforeKey &&
          isNightShiftTime(s.startsAt, s.endsAt, tz)
      )
      .map((s) => s.assigneeId)
  )];

  const nightTwoDaysAgo = recentShifts.find(
    (s) =>
      localDayKey(s.startsAt, tz) === twoDaysBeforeKey &&
      isNightShiftTime(s.startsAt, s.endsAt, tz)
  );

  return {
    previousAssigneeId,
    previousNightAssigneeId,
    lastNightAssigneeId: nightYesterday?.assigneeId ?? null,
    twoAgoNightAssigneeId: nightTwoDaysAgo?.assigneeId ?? null,
    lastNightAssigneeIds: nightYesterdayIds,
    twoAgoNightAssigneeIds: nightTwoDaysAgoIds,
  };
}
