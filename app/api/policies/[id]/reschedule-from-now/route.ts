...
export async function POST(
  ...
  try {
    ...
    const shiftsToRemove = await prisma.shift.findMany({
      where: { policyId, startsAt: { gte: cutoff }, status: ShiftStatus.PUBLISHED },
      select: { id: true },
    });
    ...
    const priorState = await buildPriorState(policyId, cutoff, timeSlots, policy.timezone);
    ...
    const createdShifts = await prisma.shift.findMany({
      where: { batchId: batch.id, startsAt: { gte: cutoff }, status: ShiftStatus.PUBLISHED },
...
async function buildPriorState(
  ...
  return {
    ...
    lastNightAssigneeId: nightYesterdayIds[0] ?? null,
    twoAgoNightAssigneeId: nightTwoDaysAgoIds[0] ?? null,
